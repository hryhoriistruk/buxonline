...
const Bot: React.FC<BotProps> = ({ apiData }) => {
  const { language } = useParams();

  const [apiEx, setApiEx] = useState<Language[] | null>(null);
    ...
    <>
      ...
      <div className="row">
        ...
        <div className="col-lg-4 m-tc">
          ...
          {apiEx && apiEx.map((el) => el.code_a2 === language ? (
            <a href='https://t.me/buxonline' className='button'>{el.page_meta.footer_bot}</a>
          ) : <></>)}
        </div>
